...
export const SunGlasses = () => {
    ...
    const fetchDataByCategory = async (category) => {
        try {
            const res = await fetch(`${process.env.REACT_APP_API_URL}/products/category/sunglasses?page=${page}${filterQuery}`, {
                method: "GET",
                headers: {
    ...
    const searchProduct = async (category) => {
        try {
            const res = await fetch(`${process.env.REACT_APP_API_URL}/products/category/sunglasses/search?page=${searchPageNumber}&searchTerm=${searchParams.get("searchTerm") ? searchParams.get("searchTerm") : ""}${filterQuery}`, {
                method: "GET",
                headers: {
                    "Content-Type": "application/json"
                }
                        ...
                        <>
                            <section className='pt-5' style={{ backgroundColor: "#eee" }}>
                                <div className='container'>
                                    <h1 className='text-center fw-light'><span className='fw-bold'>Sun</span>glasses</h1>
